...
- Есть возможность объявления процедуры через ```:``` до имени; завершение процедуры объявляется через ```;```
...
[Микропрограммы мнемоник](mnemonic_description.txt)
